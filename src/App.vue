...
import type { AppBootstrap, ContentItem, ContentTypeFilter, PageDiagnosis, SyncEvent, SyncSession, SyncSource } from "./types";
  ...
  <main class="layout">
    <section class="panel hero">
      <div>
        ...
        <h1>今日头条收藏/喜欢同步</h1>
        <p class="hint">
          先在 Chrome 打开今日头条并登录，进入“收藏”或“喜欢”列表页，再点击同步。
          默认数据目录是 `D:\toutiao-sync`；你也可以用下面的按钮改目录或迁移目录。当前实现按增量入库：已入库内容会跳过，文章保存为 HTML/JSON，视频下载到本地目录。
        </p>
      </div>
